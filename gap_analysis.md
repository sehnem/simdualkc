...
- **[DONE] Groundwater**:
    ...
    - **[VALIDATED]** Formula validated against `simET` R package v1.0.2 (`cal_capillaryRise` by Liu/Pereira, CRAN) for fallow conditions (Sim 27–32), where zr ≈ 0 makes the Dw definition difference irrelevant. In fallow, both our code and simET match `T_Resultados` perfectly. **Important caveat:** simET uses `Dw = GroundwaterDepth - rootDepth` (depth below root zone) and `ETm = (Kcb+Ke)*ET0` — both differing from our `Dw = max(wt, zr)` and `ETm = Kcb*ETo`. These differences were tested exhaustively and both simET formulations produce *worse* fits to `T_Resultados` for active crops. simET was validated only as an independent reference implementation of the Liu et al. (2006) equations — it is not a ground truth for the original Access software's undocumented behavior. **Active crops** show systematic deviations from `T_Resultados` (early-season CR=0 vs. small positive ~0.05–0.15; mid-season with shallow water table / wet soil: CR=0 vs. ~3.8 capped). These are attributed to undocumented guard conditions in the original Access VB6 software (not accessible — only compiled binaries).
