...
climate = [
    ClimateRecord(
        ...
        eto=5.0, precip=0.0, u2=1.5, rh_min=50.0
    ) for i in range(60)
]
...
ax.plot(df_sprinkler.index, df_sprinkler['evap_act'].cumsum(), label='Sprinkler ($f_w$=1.0)', color='red', lw=2.5)
ax.plot(df_drip.index, df_drip['evap_act'].cumsum(), label='Drip ($f_w$=0.3)', color='blue', lw=2.5)
